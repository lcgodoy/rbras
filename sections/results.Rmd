...
```{r}
plot_ly(df, x = ~pc1, y = ~pc2, z = ~pc3, color = ~partido,
             marker = list(size = 2)) %>%
  add_markers(alpha = .7,
              hoverinfo = 'partido') %>%
  layout(scene = list(xaxis = list(title = paste0('PC1', ' ','(', 
                                                  labs_pca[1], ')'),
                                   angle = 90),
...
- Given a clients data base with addresses, you can know about them social profile using our method
